...
Assume our existing website converts 50% of customers to paying subscribers.<br> We have a new website we think generates a lift of 101% --- or a delta-to-control = `{python} f"{true_delta_planned:.2%}"`.
...
null_average = 100
scale = 0.25
treatment_effect_samples = np.random.laplace(loc=null_average, scale=scale, size=number_simulations)
...
fig.update_xaxes(title_text='Potential Treatment Effect (ITC)',showticklabels=True, title_standoff=10)
...
all_sims = []
treatment_effect_samples = laplace(loc=null_average, scale=scale, size=number_simulations)
...
for simulation_number in range(number_simulations):
    ...
    control_events_list = cumsum(binomial(control_daily_trials, control_mean, max_number_test_days))
    treatment_events_list = cumsum(binomial(treatment_daily_trials, treatment_mean, max_number_test_days))
    ...
    observed_effect_list = treatment_events_list*control_trials_list/(control_events_list*treatment_trials_list)

    standard_error = sqrt((p_control * (1 - p_control) / control_trials_list) +
...
later_df = sim_df.copy()
del sim_df["p-value"]
del sim_df["true_effect"]



print(f"Random Treatment Effect Programmed into Treatment: {treatment_effect_actual/100:.3%}")
...
To avoid `P-hacking`, we only calculate a `p-value` at the conclusion of our test.
...
#p_values_display = ["" for i in range(len(p_values))]
#p_values_display[max_number_test_days - 1] = round(p_values[max_number_test_days - 1],3)
...
When our true Treatment Effect > 100, our significance test performs terribly.
...
Let's break the rules!!! Let's calculate a `p-value` every day of our test.<br>
...
first_day_occurrence = later_df[later_df["p-value"] < alpha].groupby("experiment_number")["Day"].min()

# Merge back to later_df
later_df["first_occurrence_sig"] = later_df["experiment_number"].map(first_day_occurrence)

later_df["first_occurrence_sig"] = later_df["first_occurrence_sig"].fillna(max_number_test_days)



first_day_occurrence_futility = later_df[later_df["p-value"] > 1- alpha].groupby("experiment_number")["Day"].min()
later_df["first_day_occurrence_futility"] = later_df["experiment_number"].map(first_day_occurrence_futility)

later_df["first_day_occurrence_futility"] = later_df["first_day_occurrence_futility"].fillna(max_number_test_days)

later_df["first_day"] = later_df[["first_occurrence_sig", "first_day_occurrence_futility"]].min(axis=1)


display(HTML(later_df[later_df.columns[1:len(later_df.columns)-4]].tail(5).to_html(index=False)))
...
small_df = later_df[later_df.Day == later_df.first_day].copy()

negative_df = small_df [small_df.true_effect < 100].copy()
...
positive_df = small_df [small_df.true_effect > 100].copy()
