...
from numpy import repeat, cumsum, round, sqrt
...
for simulation_number in range(number_simulations):
    ...
    p_values = [1 - norm.cdf(z) for z in z_score]
    inner_dict = {
        ...
        "observed delta (%)": round(delta_list*100,2),
        "z-score": z_score,
        "p-value": p_values,
...
display(HTML(sim_df.to_html(index=False)))
...
```
**Retained Control**: `{python} f"{negative_effect_accuracy:.2%}"`
...
positive_effect_accuracy = mean(where(final_df["p-value"] < 0.05 , 1, 0))

```

**Picked Treatment**: `{python} f"{positive_effect_accuracy:.2%}"`